#!/bin/bash
aws cloudformation list-exports --query 'Exports[?Name==`ward-test-security-'"$STAGE"'`].Value' --output text > rolearn.txt
aws cloudformation list-exports --query 'Exports[?Name==`ward-test-security-secrets-'"$STAGE"'`].Value' --output text > s3.txt
